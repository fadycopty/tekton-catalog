...
The file can be placed anywhere in the repository which contains the Terraform Configuartion to be analyzed.  
If placed anywhere in the repository, then the relative file path (including the file name) needs to be provided when calling the Terraform Code Analysis.  
If placed under the Terraform Configuration containing folder, the name of the file has to be `goal-list.json`. In this option, providing the file path when calling the Terraform Code is not required and the code will attempt to load the file automatically. 

## List of parameters and default values
Follows a list of parameters that can be used for SCC goals. 

**Note that not all the goals are implemented due to terraform limitations.**
